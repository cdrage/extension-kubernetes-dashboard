...
-->
<script lang="ts">
import { TableColumn, TableDurationColumn, TableRow, TableSimpleColumn, NavPage, Table, FilteredEmptyScreen, EmptyScreen, Dropdown, Tooltip } from '@podman-desktop/ui-svelte';
...
const printerColumns = $derived.by((): PrinterColumn[] => {
  ...
  const matchingVersion = matchingSpec.versions?.find((v: CrdVersionSpec) => v.name === version)
    ?? matchingSpec.versions?.find((v: CrdVersionSpec) => v.served);

  return (matchingVersion?.additionalPrinterColumns ?? [])
    .filter((col: PrinterColumn) => !SKIP_COLUMNS.has(col.name.toLowerCase()));
});
    ...
    col =>
      new TableColumn<CustomResourceInstanceUI, string>(col.name, {
        ...
        comparator: (a, b): number =>
          (a.extraFields[col.name] ?? '').localeCompare(b.extraFields[col.name] ?? ''),
      }),
  );
}
...
const columns = $derived.by((): TableColumn<any>[] => {
  const base = isNamespaced
    ? [statusColumn, nameColumn, namespaceColumn]
...
async function fetchInstances(): Promise<void> {
  try {
    ...
    instances = result.map((o: KubernetesObject) => crHelper.getCustomResourceInstanceUI(o, resolvedKind, group, version, plural, printerColumns));
...
onMount(() => {
  ...
  fetchInstances().then(() => startPolling()).catch(console.warn);
});
      ...
      {:else}
        ...
        {#if filteredInstances.length === 0 && !loading}
          ...
          {:else}
            <EmptyScreen
              icon={KubeIcon}
              title="No {resolvedKind}"
              message="No {resolvedKind} resources found" />
          {/if}
        {/if}
      {/if}
    </div>
